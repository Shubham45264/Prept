...
import { SectionLabel, GrayTitle, GoldTitle } from "@/components/reusables";
import { ONBOARDING_ROLES, CATEGORIES } from "@/lib/data";
...
const onboardingPage = () => {
  ...
  const toggleCategory = (val) => {
    setForm((prev) => ({
      ...prev,
      categories: prev.categories.includes(val) ? prev.categories.filter((c) => c !== val)
        : [...prev.categories, val],
  ...
  return (
    <div className='min-h-screen px-6 flex flex-col items-center'>
      <div className='w-full max-w-2xl'>
          ...
          <div className='grid grid-cols-2 gap-4 w-full'>
            {ONBOARDING_ROLES.map((r) => (
              <button
                ...
                className='text-left rounded-2xl p-8 border-white/10 bg-[#0f0f11] hover:border-amber-400/20 hover:-transalte-y-0.5 transition-all duration-300'
              >
        ...
        {role && <div className='flex flex-col gap-6'>
          <div className='flex items-center justify-between bg-[#ofof11] border border-white/10 rounded-2xl px-6 py-4'>
            <div className='flex items-center gap-3'>
              <span className='w-9 h-9 rounded-xl bg-amber-400/10 border border-amber-400/20 flex items-center justify-center text-base shrink-0'>
                {ONBOARDING_ROLES.find((r) => r.value === role)?.icon}</span>
              <div>
          ...
          {/* {Interviewer Form} */}
          {role === "INTERVIEWER" && (<div className='bg-[#0f0f11] border border-white/10 rounded-2xl p-8 flex flex-col gap-6'>
